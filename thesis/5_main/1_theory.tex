...
\end{equation*}
Перший ступінь наближення зникає
\begin{equation*}
  \begin{split}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
Перепишемо характеристичну функцію вектора $\eta^k$
\begin{equation*}
  \begin{split}
    \lim_{m \to \infty} \varphi_{\eta^k}\left( \lambda \right)
...
Для подальших розрахунків розглянемо стандартний гаусивський вектор
...
\end{equation*}
Подивимось, яке розподілення має наступний вектор
\begin{equation*}
  \Upsilon
  = \sum_{j=2}^{N} \hat{\xi_j} \cdot f_j
  = \hat{\xi} - \xi_1 \cdot e^k
\end{equation*}
Для цього розглянемо квадратичну форму
\begin{equation*}
  \Mean{\left( \Upsilon, \lambda \right) \cdot \left( \Upsilon, \lambda \right)}
...
\subsection{Застосування критерія узгодженості Пірсона $\chi^2$}
