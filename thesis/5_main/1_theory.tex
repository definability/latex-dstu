...
Тому скористаємось методом найменших квадратів для апроксимації результатів
моделювання.
Для того, щоб отримати нелінійну складову, треба лише ввести координатну вісь
$t^2$.
...
\end{equation*}
Поділимо обидві частини на $- a \cdot \left( t_1 - t_6 \right)$
\begin{equation*}
  \frac{b}{-a} \ge t_1 + t_6
\end{equation*}
...
\begin{equation*}
  - \varepsilon
  \ge a \cdot \left( t_6^2 - t_1^2 \right) + b \cdot \left( t_6 - t_1 \right)
  \ge \varepsilon
\end{equation*}
Знайдемо обмеження для $b$
\begin{equation*}
  a \cdot \left( t_6 + t_1 \right) + \frac{\varepsilon}{t_6 - t_1}
  \le b
  \le a \cdot \left( t_6 + t_1 \right) - \frac{\varepsilon}{t_6 - t_1}
\end{equation*}
Розглянемо той випадок, коли на проміжку $\left[ t_1, t_6 \right]$
отримана функція не монотонна.
Це означає, що свого глобального екстремуму (максимуму або мінімуму) вона
досягає на інтервалі $\left( t_1, t_6 \right)$
\begin{equation*}
  0 \le t_1 < t_{g} = \frac{-b}{2 \cdot a} < t_6
\end{equation*}
У випадку опуклої (ввігнутої) параболи в точці $t_{g}$ досягається максимум
(мінімум) функції.
Візьмемо опуклу параболу ($a < 0$).
Треба, щоб різниця між значеннями функції у найвищій та найнижчій точці не
перевищувала $\varepsilon$.
\begin{equation*}
  - \varepsilon \le
...
($t_1$ або $t_6$), яку позначимо через $t_l$.
Отримали рівняння
\begin{equation*}
...
\begin{equation*}
  b^2 + b \cdot 2 \cdot a \cdot \left( t_g + t_l \right)
    + a^2 \cdot \left( t_g + t_l \right)^2
    - \frac{\varepsilon^2}{\left( t_g - t_l \right)^2}
  \le 0
\end{equation*}
Розв’язок відносно $b$ має вигляд
\begin{equation*}
  -a \cdot \left( t_g + t_l \right) - \frac{2 \cdot \varepsilon}{t_g - t_l}
  \le b
  \le -a \cdot \left( t_g + t_l \right) + \frac{2 \cdot \varepsilon}{t_g - t_l}
\end{equation*}
...
Спростимо рівняння
\begin{equation*}
  \left( \sqrt{a} \cdot t_l + \frac{b}{2 \cdot \sqrt{a}} \right)^2
  \le \varepsilon
\end{equation*}
...
Почнемо з аналізу лінійної функції ($a=0$).
ЇЇ графік може бути прямим (відхилення від початкового положення не більше
$\varepsilon = 1$) або спадним (відхилення будь-яке, але кінець ламаної повинен
бути нижче її початку).
Інші ламані нас не цікавлять.
Отже маємо умови
\begin{equation*}
  \begin{split}
    -\varepsilon
    \le \left( b \cdot t_1 + c \right) - \left( b \cdot t_6 + c \right)
    &\le \varepsilon, \\
    \left( b \cdot t_1 + c \right) - \left( b \cdot t_6 + c \right)
      &< \varepsilon
  \end{split}
\end{equation*}
Рівняння легко спрощуються
\begin{equation*}
  \begin{split}
    -\varepsilon \le b \cdot \left( t_1 - t_6 \right) &\le \varepsilon, \\
           b \cdot \left( t_1 - t_6 \right)
      &< \varepsilon
  \end{split}
\end{equation*}
Умова рівного графіку
\begin{equation*}
  \frac{\varepsilon}{-\tau} \le b \le \frac{\varepsilon}{\tau},\qquad a = 0
\end{equation*}
Умова спадного графіку
\begin{equation*}
  b < \frac{\varepsilon}{-\tau},\qquad a = 0
\end{equation*}
Рівняння прямих, що не підходять, задовольняють умові
\begin{equation*}
  \frac{\varepsilon}{\tau} < b,\qquad a = 0
\end{equation*}
