...
(мінімум) функції.
Узагальнимо нерівність, позначивши локальний максимум як $t_{max}$, а мінімум
через $t_{min}$
\begin{equation*}
  - \varepsilon \le
...
\begin{equation*}
  -a \cdot \left( t_g + t_l \right) - \frac{\varepsilon}{t_g - t_l}
  \le b
  \le -a \cdot \left( t_g + t_l \right) + \frac{\varepsilon}{t_g - t_l}
\end{equation*}
Відомо, що $t_{g} = \frac{-b}{2 \cdot a}$.
Підставимо це значення в рівняння \eqref{eq:flat:abs}
\begin{equation*}
...
\begin{equation*}
  -2 \cdot t - 2 \sqrt{\left| a \right| \cdot \varepsilon}
  \le b
  \le -2 \cdot t + 2 \sqrt{\left| a \right| \cdot \varepsilon}
\end{equation*}
